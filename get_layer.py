...
model_layer.load_state_dict(torch.load('/media/disk8T/more2/wr/RRM/cam_model/cam-10.pth'))
model_layer.eval()
layer_name = 'layer1'
# print(model)
model_dict = dict(arch=model_layer, layer_name=layer_name, input_size=(256,256))
...
for i, pack in enumerate(pseudo_loader):
    ...
    cam_stride = []
    cam_hires = []
    zero_stride_map = np.zeros((256//4, 256//4))
    zero_hires_map = np.zeros((256, 256))
    for i in range(class_idx_indexs.shape[0]):
        class_idx_i = class_idx_indexs[i]
        if np.all(class_idx_i == 0):
            # cam_all.append(zero_map)
            cam_stride.append(zero_stride_map)
            cam_hires.append(zero_hires_map)
        else:
            cam_stride_map, cam_hires_map = model_layercam(image_fea, class_idx=class_idx_i)
            layer_cam_stride, layer_cam_hires = cam_stride_map.cpu().numpy().reshape([256//4, 256//4]), cam_hires_map.cpu().numpy().reshape([256, 256])
            # cam_all.append(layercam)
            cam_stride.append(layer_cam_stride)
            cam_hires.append(layer_cam_hires)
    
    # layer_cam = np.stack([i for i in cam_all], axis=0)
    layer_cam_stride_all = np.stack([i for i in cam_stride], axis=0)
    layer_cam_hires_all = np.stack([i for i in cam_hires], axis=0)
    valid_category = np.nonzero(class_idx)[0]
    cam_stride_save = layer_cam_stride_all[valid_category]
    # cam_hires_save = layer_cam_hires_all[valid_category]
    print(valid_category)
    print('stride尺寸', cam_stride_save.shape)
    # print('hires尺寸', cam_hires_save.shape)
    cam_stride_save_norm = cam_stride_save / (np.max(cam_stride_save, (1, 2), keepdims=True) + 1e-5)
    cam_hires_all_norm = layer_cam_hires_all / (np.max(layer_cam_hires_all, (1, 2), keepdims=True) + 1e-5)
    # print(layer_cam_norm.shape)
    # print(np.nonzero(class_idx)[0])
    # print(layer_cam_norm[np.nonzero(class_idx)[0]].shape)
    ...
    # cam_fuse_norm = cam_f / (np.max(cam_fuse, (1, 2), keepdims=True) + 1e-5)
    np.save(os.path.join('/media/disk8T/more2/wr/RRM/layer_cam_save/layer1/dict_pred/', img_name[0].replace('jpg', 'npy')), {"keys": valid_category, "cam":cam_stride_save_norm})


    seg_label = compute_seg_label_v3(ori_image, class_idx, cam_hires_all_norm) 

    np.save('/media/disk8T/more2/wr/RRM/layer_cam_save/layer1/seg_label/'+ img_name[0] + '.npy', seg_label)

    # seg_label = compute_seg_label_layer(ori_image, class_idx, cam_mat_all_norm)
    seg_label_map = visual_v2(seg_label)
    imsave('/media/disk8T/more2/wr/RRM/layer_cam_save/layer1/seg_pred_map/'+ img_name[0] + '.png', seg_label_map)
