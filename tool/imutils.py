...
def crf_inference(img, probs, t=1, scale_factor=1, labels=7):
    import pydensecrf.densecrf as dcrf
    from pydensecrf.utils import unary_from_softmax

    h, w = img.shape[:2]
    # print('图像尺寸', img.shape[:2])
    # print('高', h)
    # print('宽', w)
    n_labels = labels
    ...
    d.addPairwiseBilateral(sxy=150/scale_factor, srgb=50, rgbim=np.copy(img_c), compat=5)
    Q = d.inference(t)
